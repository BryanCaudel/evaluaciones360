...
if ($relacion === 'Autoevaluación') {
    $stmt_total->bind_param("ii", $evaluacion_id, $evaluacion_id);
} else {
    $stmt_total->bind_param("iiss", $evaluacion_id, $evaluacion_id, $nombre, $relacion);
}

$stmt_total->execute();
$pendientes = $stmt_total->get_result()->fetch_assoc()['total'];
...
<head>
    <meta charset="UTF-8">
    <title>Evaluación en Progreso</title>
    ...
    <style>
        ...
        /* Barra del slider más delgada */
input[type="range"] {
    width: 100%;
    height: 12px; /* delgada pero visible */
    background: linear-gradient(to right, red, orange, yellow, lightgreen, green);
    border-radius: 6px;
    appearance: none;
...
.slider-labels {
    display: flex;
    justify-content: space-between;
    font-size: 16px; /* más grande que antes */
    font-weight: bold;
    margin-top: 8px;
    color: #333;
}
...
<body>
    ...
    <div class="info">
        ...
        <p><strong>Preguntas mostradas:</strong> <?= count($preguntas) ?> / Pendientes totales: <?= $pendientes ?></p>
